...
const _TBModule  = require('node-telegram-bot-api');
const TelegramBot = _TBModule.default || _TBModule;
...
app.use((_, res, next) => {
  ...
  res.setHeader('X-XSS-Protection', '0'); // Deprecated; CSP below supersedes it
  res.setHeader('Referrer-Policy', 'strict-origin-when-cross-origin');
  res.setHeader(
    'Content-Security-Policy',
    [
      "default-src 'self'",
      "script-src 'self' 'unsafe-inline'",           // inline JS in index.html (refactor target)
